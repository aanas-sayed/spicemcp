...
def test_ngspice_config_from_json(tmp_path):
    cfg_file = tmp_path / "config.json"
    cfg_file.write_text(
        json.dumps({"simulators": {"ngspice": {"image": "my-custom/ngspice:v1", "timeout_s": 120}}})
    )
    config = load_config(path=cfg_file)
...
def test_local_override_takes_precedence(tmp_path):
    ...
    cfg_file.write_text(
        json.dumps(
            {
                "simulators": {"ltspice": {"backend": "docker", "image": "docker-image:v1"}},
                "overrides": {"ltspice": {"backend": "local", "executable": "/usr/bin/ltspice"}},
            }
        )
    )
    config = load_config(path=cfg_file)
